...
# Steven Xijin Ge  South Dakota State University; modified by Sara Di Rienzi (2020)
# gexijin@gmail.com
# 3/12/2018
...
geneInfo <- function (fileName = NULL){
	...
	{ x = read.delim(as.character(fileName) ); x[,1]= toupper(x[,1]) } else # read in the chosen file 
...
# Define sample groups based on column names
 ...
 densityPlotraw <- function() {
   ...
   myColors = mycolors
   plot(density(x[,1]),col = myColors[1], lwd=2,
        xlab="Expresson values", ylab="Density", main= "Distribution of raw data",
         xlim=c(0,1000) )
 ...
 genePlot <- function() {
	...
	#merge with metadata
	mergeddata<-merge(x=mdf,y= metadata,by.x="samples",by.y="samples")
#	mdf <- as.data.table(summarySE((mdf, measurevar = "value", groupvars = c("induction", "treatmentreplicate_")))
	

	p1 <- ggplot(data=mdf, aes(x=samples, y=value,group = Genes, shape=Genes, colour = samples)) +
		geom_line() +
...
geneBarPlotError <- function() {
   x <- convertedData.out
	...
	# matching from the beginning of symbol
	if (exactmatch == FALSE) { #if don't want an exact mathc
	  ix = which(regexpr(  paste("^" , toupper(input_geneSearch),sep="")   ,toupper(x$Genes)) > 0)
	}
...
geneBarPlotError2 <- function(normreadcountdata) {
  x <- normreadcountdata
...
geneexpressiondata <- function() {
  ...
  #merge with metadata
  mergeddata<-merge(x=mdf,y= metadata,by.x="samples",by.y="samples")
test<-wilcox.test(value~induction_,data=mergeddata)
genepvalue<-c(gene,test$p.value)
genepvalue
}

geneexpressiondata2 <- function(normcountdata) {
  x <- normcountdata
...
PCAplot <- function() {   #PCA
	...
	if(is.null(readSampleInfo.out)) { 
		...
		p=ggplot(pcaData, aes_string("PC1", "PC2", fill=input_selectFactors,color=input_selectFactors,shape=input_selectFactors2)) + stat_ellipse()+geom_point(size=5,color="black") 
...
MDSplot <- function(dimen1, dimen2) {  # MDS
	 	x <- convertedData.out;
	 fit = cmdscale( dist(t(x) ), eig=T, k=4)  #k is the max number of dimensions you can plot
	 dim1<-paste("x",dimen1,sep="")
	 dim2<-paste("x",dimen2,sep="")
	 
	 
	# par(pin=c(5,5))
	pcaData = as.data.frame(fit$points[,1:4]); pcaData = cbind(pcaData,detectGroups(colnames(x)) )
	colnames(pcaData) = c("x1", "x2","x3","x4", "Sample_Name")
	...
	p=p+xlab(paste("Dimension ",as.character(dimen1),sep="")) +
	  ...
	  scale_fill_manual(values=c("gold","orchid1","purple2"))+
	  scale_shape_manual(values=c(21,24))
	p=p+ylab(paste("Dimension ",as.character(dimen2),sep="")) 
...
annotateDEGswithfunction<- function() {		
  NoSig=NULL
  ...
  fc = selectedHeatmap.data.out$bar		#up or down regulated
  # GO
  ...
  for( i in c(1,-1) ) {
    ...
    result <- annotateDEGs( query, GeneSets.out,1)
    return(result)
  }
}

annotateDEGs<- function ( query, geneSet, minFDR=.2 ,minSize=2,maxSize=10000 ){ 
  #geneSets <- readGMT("exampleData/MousePath_TF_gmt.gmt")
  #query <-  geneSets[['TF_MM_FRIARD_C-REL']] 
  #query <- query[1:60]
  total_elements = 30000
  Min_overlap <- 1
  maxTerms =20 # max number of enriched terms
  ...
  geneannotations<-unlist( lapply(geneSet, function(x)  intersect (query, x) ) ) 
  DEGgenes<-unique(geneannotations)
  DEGannotations<-data.frame(DEGname = character(),DEGgenefunction = list())
  for (gene in DEGgenes) {
    geneannotationsfound<-names(which(geneannotations == gene))
    geneannotationsfound<-paste(geneannotationsfound,collapse = ', ')
    tempframe<-data.frame(DEGname = gene, DEGgenefunction = geneannotationsfound)
    DEGannotations<-rbind.data.frame(DEGannotations,tempframe)
  }
  return(DEGannotations)
}

DEGfunctionannotation <- function() {		
  NoSig=NULL
  # using expression data
  genes <- selectedHeatmap.data.out$genes
  if(is.null(genes) ) return(NULL) 
  ...
  for( i in c(1,-1) ) {
    ...
    result <- findOverlapGMTwithgenes( query, GeneSets.out,1)
    return(result)
  }
  
}
  
findOverlapGMTwithgenes<-  function ( query, geneSet, minFDR=.2 ,minSize=2,maxSize=10000 ){ 
  #geneSets <- readGMT("exampleData/MousePath_TF_gmt.gmt")
  #query <-  geneSets[['TF_MM_FRIARD_C-REL']] 
  #query <- query[1:60]
  total_elements = 30000
  ...
  geneSet <- geneSet[which(sapply(geneSet,length) > minSize)]  # gene sets smaller than 1 is ignored!!!
  geneSet <- geneSet[which(sapply(geneSet,length) < maxSize)]  # gene sets smaller than 1 is ignored!!!
  resultswithgenes <-unlist( lapply(geneSet, function(x) intersect (query, x) ))
  
  return(resultswithgenes)
}
...
DEG.DESeq2 <- function (  rawCounts,maxP_limma=.05, minFC_limma=2, selectedComparisons=NULL, sampleInfo = NULL,modelFactors=NULL, blockFactor = NULL, referenceLevels=NULL){
	...
	# remove samples without replicates
	g <- rownames(reps)[which(reps[,1] >1)]
	ix <- which( groups %in% g)  #pulling up rows with groups that have the correct # of replicates
	groups <- groups[ix]   #could subset here!!!
	...
	for( i in 1:(length(g)-1) )
		for (j in (i+1):length(g)) 
		comparisons = c(comparisons,paste(g[j],"-",g[i],sep="" ) )
	comparisons <- comparisons[-1]   #all possible comparisons!

	colData = cbind(colnames(rawCounts), groups )   #adding metadata to the sample names
	...
	{    # using selected factors and comparisons
		...
		colData = as.data.frame(colData)  #metadata
	...
	for( kk in 1:length(comparisons) ) {
		...
		totaloutput<-selected #use this to printout the total DESEQ2 output; can repeat for each comparison
		selected$calls =0   
		selected$calls [which( selected$log2FoldChange > log2(minFC_limma) & selected$padj < maxP_limma ) ]  <-  1  #up reg
		selected$calls [ which( selected$log2FoldChange <  -log2(minFC_limma) & selected$padj < maxP_limma ) ] <-  -1  #down reg
		...
		topGenes[[1]] = selected[,c(2,6)];   # this selects what data will be used = fold change and padj
		names(topGenes)[1] = comparisons2[kk]; } else 
			{ result1 = merge(result1,selected,by="row.names"); 
				...
				names(topGenes)[pk] = comparisons2[kk];  # assign name to comparison
			}
	}
					...
					for(interactionTerm in allInteractionTerms ) {
						...
						if (pp==0){  # if first one with significant genes, collect gene list and Pval+ fold
							...
							{ result1 = merge(result1,selected,by="row.names"); 
								...
								# selected[,2] <- -1 * selected[,2] # reverse fold change direction
								topGenes[[pk]] = selected[,c(2,6)]; 
								names(topGenes)[pk] = comparisonName;  # assign name to comparison
							}
					} #for	
...
annotatedDEG<- function () {
  allgenes = limma.out$results
  DEGgenes = as.data.frame( genes[which( rowSums(abs(genes)) != 0 ),] )  #added Abs here because losing data otherwise
  DEGgenes$genename = row.names(DEGgenes)
  #add all information and then subset on the other page
 humangeneinfo<-
  # humangeneinfo<-read.delim(file="humangenome_export3.txt",header=TRUE, sep="\t") #downloaded from http://www.ensembl.org/biomart/martview/5ce7f7cffd709bb1f39037c7785482b9  this is GRCh37, current is GRCh38
  #humangeneinfo<-read.delim(file="readcount_genename.txt",header=TRUE, sep="\t") #supplied with readcounts
  #humangeneinfo<-humangeneinfo[,c(1,38:42,44:45)]
  #genedataupdate<-merge(x=DEGgenes,y=humangeneinfo,by.x = "row.names",by.y = "Gene") #.stable.ID",all.x=TRUE)
  genedataupdate<-merge(x=DEGgenes,y=humangeneinfo,by.x = "row.names",by.y = "Gene.stable.ID",all.x=TRUE) #.stable.ID"
  names(genedataupdate)[1] = "EnsemblnameGRCh37"
  #write.csv( genedataupdate,'outcheckallDEGsensembl.csv') 
  
  length( which(genedataupdate$Gene.stable.ID %in% row.names(convertedData.out) ))
  return(genes)
}
...
DEG.data <- function() {
		  ...
		  genes = as.data.frame( genes[which( rowSums(abs(genes)) != 0 ),] )  #added Abs here because losing data otherwise
		  colnames(genes) = colnames( limma.out$results )
		  genes = merge(genes,convertedData.out, by='row.names')   #merged with gene expression values (log2 stuff)
...
DEG.datafixed <- function() {
 ...
 # ix = match( genes[,1], allGeneInfo.out[,1])
  #genes <- cbind(as.character( allGeneInfo.out$symbol)[ix],genes) 
  #colnames(genes)[1] = "Symbol"
  genesfunction = merge(genes,allGeneInfo.out,by.x="Row.names", by.y="ensembl_gene_id", all.x=TRUE)
  #colnames(genes)[1] = "1: upregulation, -1: downregulation"
 # genes <- genes[,c(2,1,3:dim(genes)[2]) ]
  return(genesfunction)
}
...
geneListGOTable <- function() {		
		...
		for( i in c(1,-1) ) {
			...
			result <- findOverlapGMT( query, GeneSets.out,1)  #genesets is the full list
		...
		results1= results1[,c(6,1,2,3,4,5)] # changed
		...
		if(dim(results1)[2] != 6) return(NoSig)  # changed
...
fgseaPathwayData <- function() {
    ...
    { top1=limma.out$topGenes[[1]]  
	  ...
	  gmt = GeneSets.out #add pathway organization to gene list
     if(length( GeneSets.out )  == 0)  { return(as.data.frame("No gene set found!"))}
	 ...
	 paths <- fgsea(pathways = gmt,   #total set of genes
                  stats = fold,    #just those in my data with fold change
	 ...
	 # paths <-  rbind(paths$greater,paths$less)
	  if(dim(paths)[1] < 1  ) return( noSig )
       ...
       paths <- paths[order(-abs( paths[,5])) ,]  # sort by NES     #print out paths here to get pathway, padju, genes involved
...
if (is.null(input_selectContrast1 ) ) return(NULL)
	...
	show.grid = T, margins = c(3,1, 13,20), col = .rwb,cex.lab=0.5, main="Pathway Analysis:PGSEA")
    }
...
pathwayListData  <- function(){
	...
	for(i in 1:nrow(pathways) ){ 
      ...
      else if (length(ix) > 1) {       #added this part
        genesmulti1<-NULL
        genesmulti2<-NULL
       for (ixa in 1:length(ix)) {
         genes <- GeneSets.out[[ixa]] # retrieve genes
         genesmulti1<-c(genesmulti1,genes)
         if(!is.null(probeToGene) ) { 
           iy = match(genes,probeToGene[,1])
           genes = probeToGene[iy,2]
           genesmulti2<-c(genesmulti2,genes)
          }
       }
        pathways$Genes[i] = paste(genesmulti1, collapse=" ")
        if (!is.null(genesmulti2 ) ) { pathways$Genes[i] = paste(genesmulti2, collapse=" ") }
